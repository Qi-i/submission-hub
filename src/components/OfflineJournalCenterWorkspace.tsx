...
export default function OfflineJournalCenterWorkspace({ refreshToken, onChanged }: Props) {
  ...
  return <section className="journal-center-workspace" aria-label="期刊中心">
    {uiMode === 'luminous-x' && <LuminousXStatusBar modeLabel="期刊中心" subtitle="集中管理期刊档案、投稿入口、评价指标、费用与横向比较。" recordCount={snapshot.journals.length} />}
    ...
    <div className="journal-center-grid paper-grid journal-catalog-grid">{visible.map(journal=><JournalCatalogCard key={journal.id} journal={journal} onClick={()=>setEditor(journal)} standalone thirdPartyLinkLimit={(journal.third_party_links || []).length}/>)}{!visible.length && <div className="journal-center-empty">暂无符合条件的期刊。</div>}</div>
