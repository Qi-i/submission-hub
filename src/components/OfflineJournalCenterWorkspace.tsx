...
export default function OfflineJournalCenterWorkspace({ refreshToken, onChanged }: Props) {
  ...
  return <section className="journal-center-workspace preparation-workspace" data-section="match" aria-label="期刊中心">
    ...
    <div className="journal-center-grid prep-card-grid journal-grid">{visible.map(journal=><JournalCatalogCard key={journal.id} journal={journal} onClick={()=>setEditor(journal)} standalone thirdPartyLinkLimit={(journal.third_party_links || []).length}/>)}{!visible.length && <div className="journal-center-empty">暂无符合条件的期刊。</div>}</div>
