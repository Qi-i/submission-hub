...
function Card({ journal, onEdit }: { journal: JournalProfile; onEdit: () => void }) {
  ...
  return <article className="journal-center-card">
    <button className="journal-center-card__body" type="button" onClick={onEdit}>
      ...
      <div className="journal-center-card__facts"><span>{oa}</span>{journal.indexing.slice(0, 4).map(item => <span key={item}>{item}</span>)}</div>
      <div className="journal-center-card__metrics"><span><b>{journal.first_decision_days ?? '—'}</b><small>首轮决定/天</small></span><span><b>{journal.total_review_days ?? '—'}</b><small>总审稿/天</small></span><span><b>{journal.acceptance_rate != null ? `${journal.acceptance_rate}%` : '—'}</b><small>接收率</small></span></div>
    </button>
    <div className="journal-center-card__links">{safeUrl(journal.website_url) && <a href={journal.website_url!} target="_blank" rel="noopener noreferrer">官网 <ExternalLink size={11} /></a>}{safeUrl(journal.author_guide_url) && <a href={journal.author_guide_url!} target="_blank" rel="noopener noreferrer">指南 <ExternalLink size={11} /></a>}{safeUrl(journal.submission_url) && <a href={journal.submission_url!} target="_blank" rel="noopener noreferrer">投稿 <ExternalLink size={11} /></a>}</div>
  </article>
}
