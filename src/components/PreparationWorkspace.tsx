  ...
  return <div className="preparation-workspace" data-section={section}>
    ...
    {section === 'overview' && <>
      ...
      <div className="prep-overview-grid">
        ...
        <section className="prep-panel prep-panel-journal prep-overview-journals"><PanelHead title="收藏期刊" subtitle="按期刊类型突出主要分区、核心收录、费用与审稿速度" onClick={() => setSection('journals')} /><div className="prep-overview-journal-list">{orderedJournals.slice(0, 4).map(journal => <JournalRow key={journal.id} journal={journal} onClick={() => setEditor({ type: 'journal', value: journal })} />)}</div>{!orderedJournals.length && <Empty text="尚未收藏期刊" action="收藏期刊" onClick={() => setEditor({ type: 'journal', value: 'new' })} />}</section>
      </div>
...
function DraftCard({ draft, topic, journals, primaryJournal, onEdit, onPromote, promoting, compact }: { draft: ManuscriptDraft; topic?: ResearchTopic; journals: JournalProfile[]; primaryJournal?: JournalProfile; onEdit: () => void; onPromote?: () => void; promoting?: boolean; compact?: boolean }) {
  ...
  return <article className={`prep-draft-card ${compact ? 'compact' : ''}`}><button className="prep-draft-main" onClick={onEdit}><div className="prep-draft-head"><div><span>{stage}</span>{topic && <em>{topic.title}</em>}</div><b>{readiness.score}%</b></div><h3>{draft.title}</h3><div className="prep-progress"><span style={{ width: `${readiness.score}%` }} /></div><div className="prep-draft-meta"><span>就绪度 {readiness.score}%</span><span>必需检查 {checkProgress}%</span>{draft.target_word_count ? <span>写作 {draft.current_word_count}/{draft.target_word_count}（{wordProgress}%）</span> : <span>字数 {draft.current_word_count}</span>}<span>图 {draft.figure_count} · 表 {draft.table_count} · 文献 {draft.reference_count}</span></div><div className={`prep-readiness-note ${readiness.blockers.length ? 'blocked' : readiness.score >= 90 ? 'ready' : ''}`}><strong>{readiness.nextAction}</strong>{readiness.blockers[0] && <span>{readiness.blockers[0]}</span>}</div><div className="prep-draft-foot"><span>{primaryJournal ? `主投：${primaryJournal.name}` : journals.length ? `备选 ${journals.length} 本` : '未选期刊'}</span>{days !== null && <span className={days < 0 ? 'danger' : days <= 14 ? 'warn' : ''}>{days < 0 ? `计划已逾期 ${-days} 天` : `计划 ${days} 天后投稿`}</span>}</div></button>{onPromote && !draft.submitted_paper_id && <button className="btn btn-primary btn-sm prep-promote" onClick={onPromote} disabled={promoting}>{promoting ? '转入中...' : <>确认已投出 <ArrowRight size={13} /></>}</button>}</article>
}
...
function RankBlocks({ journal, limit = 6, className = '' }: { journal: JournalProfile; limit?: number; className?: string }) {
  ...
  return <div className={`prep-journal-rank-blocks ${className}`}>{ranks.map(item => <span key={`${item.key}-${item.value}`} data-tone={journalRankTone(item.key)}><b>{item.label}</b><em>{item.value}</em></span>)}</div>
}
...
function JournalRow({ journal, onClick }: { journal: JournalProfile; onClick: () => void }) {
  ...
  return <button className="prep-journal-overview-card" onClick={onClick}>
    <div className="prep-overview-journal-head"><div className="prep-overview-journal-title"><span className="prep-journal-star">{journal.is_favorite ? '★' : '☆'}</span><span><b>{journal.name}</b><small>{journal.publisher || journal.scope || '未填写出版社或范围'}</small></span></div><span className={`prep-risk ${journal.risk_level}`}>{journal.risk_level === 'warning' ? '预警' : journal.risk_level === 'watch' ? '关注' : '正常'}</span></div>
    <RankBlocks journal={journal} limit={6} />
    <div className="prep-overview-journal-meta"><span data-tone="fit">{journalFitSummary(journal)}</span><span data-tone="oa">{oa}</span><span data-tone="speed">首轮 {journal.first_decision_days ?? '—'} 天</span><span data-tone="index">{indexing}</span></div>
  </button>
}

function JournalCard({ journal, onClick }: { journal: JournalProfile; onClick: () => void }) {
  const oa = OA_OPTIONS.find(item => item.key === journal.oa_type)?.label || '未确认'
  return <article className="prep-journal-card"><button className="prep-journal-card-main" onClick={onClick}><div className="prep-card-top"><span className={`prep-priority ${journal.priority}`}>{journal.is_favorite ? <Star size={13} fill="currentColor" /> : '未收藏'}</span><span className={`prep-risk ${journal.risk_level}`}>{journal.risk_level === 'warning' ? '预警' : journal.risk_level === 'watch' ? '关注' : '正常'}</span></div><h3>{journal.name}</h3><p>{journal.publisher || journal.scope || '尚未填写出版社与期刊范围'}</p><RankBlocks journal={journal} limit={7} className="full" /><div className="prep-journal-facts"><span data-tone="oa">{oa}</span>{journal.indexing.slice(0, 4).map(item => <span key={item} data-tone="index">{item}</span>)}</div><div className="prep-journal-numbers"><div><b>{journal.first_decision_days ?? '—'}</b><small>首轮决定/天</small></div><div><b>{journal.total_review_days ?? '—'}</b><small>总审稿/天</small></div><div><b>{journal.acceptance_rate != null ? `${journal.acceptance_rate}%` : '—'}</b><small>接收率</small></div><div><b>{journal.apc_amount != null ? journal.apc_amount : '—'}</b><small>{journal.apc_currency || 'APC'}</small></div></div></button><div className="prep-journal-links">{safeUrl(journal.website_url) && <a href={journal.website_url!} target="_blank" rel="noopener noreferrer">官网 <ExternalLink size={11} /></a>}{safeUrl(journal.author_guide_url) && <a href={journal.author_guide_url!} target="_blank" rel="noopener noreferrer">指南 <ExternalLink size={11} /></a>}{safeUrl(journal.submission_url) && <a href={journal.submission_url!} target="_blank" rel="noopener noreferrer">投稿 <ExternalLink size={11} /></a>}{journal.third_party_links.slice(0, 2).map(link => <a key={`${link.label}-${link.url}`} href={link.url} target="_blank" rel="noopener noreferrer">{link.label} <ExternalLink size={11} /></a>)}</div></article>
}
