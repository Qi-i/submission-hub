...
import { STATUSES, JCR_OPTIONS, CAS_OPTIONS, SUBMISSION_SYSTEM_OPTIONS, NEXT_ACTION_OPTIONS, REMINDER_LEVELS } from '../lib/types'
...
function initialForm(paper: Paper | 'new', currentUsername: string): FormState {
  if (paper === 'new') {
    return {
  ...
  return {
    ...
    next_action: paper.next_action || '', reminder_level: paper.reminder_level || 'none', apc_amount: text(paper.apc_amount), apc_currency: paper.apc_currency || 'USD',
...
function latestTimelineState(timeline: string, fallbackStatus: string, fallbackDate: string) {
  ...
  if (!lines.length) return { status: fallbackStatus || null, date: fallbackDate || null }
  ...
  const status = rest.split(/\s+-\s+/)[0]?.trim() || fallbackStatus || null
  return { status, date }
}

export default function PaperFormEnhanced({ paper, allPapers, currentUsername, onSave, onDelete, onClose }: Props) {
  const isNew = paper === 'new'
  const [customTlOpts, setCustomTlOpts] = useState<string[]>([])
  const [uploading, setUploading] = useState<number | null>(null)
  const [form, setForm] = useState<FormState>(() => initialForm(paper, currentUsername))
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      next_action: form.next_action || null, reminder_level: form.reminder_level || 'none',
  ...
  const updateFile = (idx: number, field: 'n' | 'p', val: string) => setForm(prev => { const files = [...prev.files]; files[idx] = { ...files[idx], [field]: val }; return { ...prev, files } })
  ...
  return <div className="modal-overlay" onClick={onClose}><div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 920 }}>
    <div className="modal-header"><h3 className="modal-title">{isNew ? '✨ 新建投稿记录' : '✏️ 编辑投稿记录'}</h3><button className="btn btn-ghost btn-icon" onClick={onClose}><X size={18} /></button></div>
    <div className="modal-body">
      <div className="field-row" style={{ gridTemplateColumns: '1fr 1fr' }}>
        <div className="field"><label className="field-label">文章语言</label><select className="select" value={form.lang} onChange={e => set('lang', e.target.value)}><option value="zh">ZH 中文</option><option value="en">EN 英文</option></select></div>
        <div className="field"><label className="field-label">当前主状态</label><select className="select" value={form.status} onChange={e => set('status', e.target.value)} style={{ fontWeight: 700, color: 'var(--accent)' }}>{STATUSES.map(s => <option key={s.key} value={s.key}>{s.emoji} {s.label}</option>)}</select></div>
      </div>
      <div className="field"><label className="field-label">论文主要标题 *</label><input className="input" value={form.title} onChange={e => set('title', e.target.value)} style={{ fontSize: 14, fontWeight: 700 }} placeholder="输入论文标题" /></div>
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: 'var(--radius-lg)', padding: 16, border: '1px solid var(--border-subtle)' }}>
        <div style={{ fontSize: 12, fontWeight: 800, marginBottom: 6, color: 'var(--text-secondary)' }}>🧭 投稿系统跟踪</div>
        <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 12 }}>最新投稿状态将自动取自“审稿时间线”的最后一条记录，不需要重复填写。</div>
        <div className="field-row" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
          <div className="field"><label className="field-label">稿件编号</label><input className="input" value={form.manuscript_no} onChange={e => set('manuscript_no', e.target.value)} placeholder="Submission ID" /></div>
        ...
        <div className="field-row" style={{ gridTemplateColumns: 'repeat(4, 1fr)', marginTop: 10 }}>
          <div className="field"><label className="field-label">下一步行动</label><select className="select" value={form.next_action} onChange={e => set('next_action', e.target.value)}><option value="">-- 自动判断 --</option>{NEXT_ACTION_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}</select></div>
          ...
          <div className="field"><label className="field-label">APC / 版面费</label><div style={{ display: 'grid', gridTemplateColumns: '1fr 72px', gap: 6 }}><input type="number" min="0" className="input" value={form.apc_amount} onChange={e => set('apc_amount', e.target.value)} placeholder="金额" /><input className="input" value={form.apc_currency} onChange={e => set('apc_currency', e.target.value.toUpperCase())} /></div></div>
        </div>
      </div>

      <div className="field"><label className="field-label">版本追溯</label><select className="select" value={form.prev_id} onChange={e => set('prev_id', e.target.value)}><option value="">-- 常规首投 --</option>{allPapers.filter(p => isNew || p.id !== (paper as Paper).id).map(p => <option key={p.id} value={p.id}>{p.title || '（未命名）'} [{p.journal || '未知期刊'}]</option>)}</select></div>
      {form.lang === 'en' ? <div className="field-row" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}><div className="field"><label className="field-label">JCR 分区</label><select className="select" value={form.quartile_jcr} onChange={e => set('quartile_jcr', e.target.value)}>{JCR_OPTIONS.map(o => <option key={o}>{o}</option>)}</select></div><div className="field"><label className="field-label">中科院分区</label><select className="select" value={form.quartile_cas} onChange={e => set('quartile_cas', e.target.value)}>{CAS_OPTIONS.map(o => <option key={o}>{o}</option>)}</select></div><div className="field"><label className="field-label">新锐分区</label><input className="input" value={form.quartile_new} onChange={e => set('quartile_new', e.target.value)} /></div><div className="field"><label className="field-label">自定义分类</label><input className="input" value={form.quartile_cust} onChange={e => set('quartile_cust', e.target.value)} /></div></div> : <div className="field-row" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>{[0, 1, 2, 3].map(i => <div className="field" key={i}><label className="field-label">{i < 3 ? `分类 / 基金 ${i + 1}` : '其它属性'}</label><input className="input" value={form.quartile_zh[i] || ''} onChange={e => { const arr = [...form.quartile_zh]; arr[i] = e.target.value; set('quartile_zh', arr) }} /></div>)}</div>}
      ...
      <div className="field-row" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}><div className="field"><label className="field-label">首投日期</label><input type="date" className="input" value={form.submitted_date} onChange={e => set('submitted_date', e.target.value)} /></div><div className="field"><label className="field-label">终审/录用日期</label><input type="date" className="input" value={form.resolve_date} onChange={e => set('resolve_date', e.target.value)} /></div><div className="field"><label className="field-label">修回截止日期</label><input type="date" className="input" value={form.deadline} onChange={e => set('deadline', e.target.value)} /></div></div>
      <div className="field"><label className="field-label">📅 审稿时间线</label><Timeline value={form.timeline} onChange={val => set('timeline', val)} customOpts={customTlOpts} onAddCustomOpt={opt => setCustomTlOpts(prev => [...prev, opt])} /></div>
      <div style={{ background: 'var(--bg-card)', borderRadius: 'var(--radius-lg)', padding: 16, border: '1px dashed var(--border-default)' }}><div style={{ fontSize: 12, fontWeight: 700, marginBottom: 12, color: 'var(--text-secondary)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span>📎 文件归档</span><button className="btn btn-ghost btn-sm" onClick={addFile} style={{ fontSize: 11 }}>+ 添加文件</button></div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>{form.files.map((f, i) => <div key={i} className="file-box"><div className="file-icon" onClick={() => upload(i)}>{uploading === i ? <Loader size={16} className="spinner" /> : <FileText size={16} />}</div><div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0 }}><input className="input" style={{ padding: '4px 6px', fontSize: 12, fontWeight: 600 }} value={f.n} onChange={e => updateFile(i, 'n', e.target.value)} placeholder="文件名称" /><input className="input" style={{ padding: '2px 6px', fontSize: 10, color: 'var(--text-muted)' }} value={f.p} onChange={e => updateFile(i, 'p', e.target.value)} placeholder="文件路径或URL" /></div><button className="file-action-btn" style={{ color: '#ef4444' }} onClick={() => removeFile(i)}><Trash2 size={13} /></button></div>)}</div></div>
      ...
      <div className="field"><label className="field-label">备注 / 意见</label><textarea className="textarea" value={form.notes} onChange={e => set('notes', e.target.value)} placeholder="记录审稿意见、修改要点等..." /></div>
    </div>
    <div className="modal-footer">{!isNew ? <button className="btn btn-danger btn-sm" onClick={() => onDelete((paper as Paper).id)}><Trash2 size={14} /> 删除记录</button> : <div />}<div style={{ display: 'flex', gap: 10 }}><button className="btn btn-ghost" onClick={onClose}>取消</button><button className="btn btn-primary" onClick={handleSave}><Save size={14} /> 确认保存</button></div></div>
  </div></div>
}
