...
export default function FigureExportPanel({ project, busy, onExportSettings, onApplyPreset, onExport }: Props) {
  ...
  return <section className="figure-composer__section figure-composer__export-panel" aria-label="出版尺寸与导出">
    <div className="figure-composer__section-title"><Ruler size={14} /><strong>出版尺寸与导出</strong><span>{outputWidth}×{outputHeight}px</span></div>
    <label className="figure-composer__export-preset"><span>预设</span><select defaultValue="custom" onChange={event => {
    ...
    <div className="figure-composer__field-grid three figure-composer__export-grid">
    ...
    <div className="figure-composer__export-actions">
      <span title={`逻辑画布 ${Math.round(project.canvas.width)} × ${Math.round(project.canvas.height)} px`}>{project.exportSettings.physicalWidth}{project.exportSettings.unit} · {project.exportSettings.dpi} DPI</span>
      <button type="button" disabled={busy} onClick={onExport}><Download size={13} /> {busy ? '生成中…' : '导出'}</button>
    </div>
  </section>
}
