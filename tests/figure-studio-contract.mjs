...
const projectIndex = sidebar.indexOf('本地草稿库')
assert(layersIndex >= 0, 'Figure Composer left rail must include 图片与图层')
assert(projectIndex >= 0, 'Figure Composer left rail must expose 本地草稿库')
assert(layersIndex < projectIndex, '图片与图层 must appear before project metadata because importing images is the first workflow step')
...
assert(canvas.includes('marquee'), 'canvas must support drag-box marquee selection for multi-panel alignment')
assert(canvas.includes('onSelectPanels'), 'canvas marquee must report multiple selected panel ids')
