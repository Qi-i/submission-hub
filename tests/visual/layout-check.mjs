...
try {
  ...
  if (await journalCenter.count()) {
    ...
    const journalTabGeometry = await page.evaluate(({ tol, reference }) => {
      ...
      if (columns !== reference.columns) failures.push(`Journal Center column count diverges from Submission Management (${columns}/${reference.columns})`)
      ...
      cards.forEach((card, index) => {
        ...
        if (Math.abs(rect.width - reference.firstCardWidth) > 5) failures.push(`journal ${index + 1}: card width diverges from Submission Management`)
