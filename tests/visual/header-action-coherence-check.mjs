...
try {
  ...
  if (!geometry.imported || !geometry.exported || Math.abs(geometry.imported.top - geometry.exported.top) > 1) fail('luminous-x/dashboard: 导入与导出未处于同一行')
  if (!geometry.feedback || !geometry.utility || geometry.feedback.width < geometry.utility.width - 2) fail('luminous-x/dashboard: 项目反馈按钮未填满工具区空白行')
  if (!geometry.primary || !geometry.filter || geometry.primary.bottom > geometry.filter.top + 1) fail('luminous-x/dashboard: 新建投稿未移动到检索筛选上方')
  ...
  const popup = await dashboard.evaluate(() => {
    ...
    return menu && trigger ? { menu: menu.toJSON(), trigger: trigger.toJSON(), viewport: { width: window.innerWidth, height: window.innerHeight } } : null
  })
  if (!popup) fail('luminous-x/dashboard: 项目与反馈弹窗未渲染')
  else {
    if (popup.menu.top < 0 || popup.menu.left < 0 || popup.menu.right > popup.viewport.width || popup.menu.bottom > popup.viewport.height) fail('luminous-x/dashboard: 项目与反馈弹窗仍被视口裁切')
...
try {
  const railGeometry = await preparationLuminous.evaluate(() => {
    const rail = document.querySelector('.preparation-workspace > .preparation-business-rail')
    if (!rail) return null
    const buttons = Array.from(rail.querySelectorAll(':scope > button')).map(element => {
      const style = getComputedStyle(element)
      const rect = element.getBoundingClientRect()
      return {
    ...
    return { buttons, rail: rail.getBoundingClientRect().toJSON(), overflow: rail.scrollWidth - rail.clientWidth }
  })

  if (!railGeometry || railGeometry.buttons.length !== 5) fail(`luminous/preparation: 核心工作区可见按钮数量错误（${railGeometry?.buttons.length ?? 0}）`)
  else {
    const tops = railGeometry.buttons.map(item => item.top)
    const heights = railGeometry.buttons.map(item => item.height)
    if (Math.max(...tops) - Math.min(...tops) > 1) fail('luminous/preparation: 五个核心工作区未处于同一行')
    if (Math.max(...heights) - Math.min(...heights) > 1 || Math.max(...heights) > 36) fail(`luminous/preparation: 核心工作区按钮高度异常（${heights.join(', ')}）`)
    for (const item of railGeometry.buttons) {
      if (item.display === 'none' || item.visibility === 'hidden' || item.pointerEvents === 'none') fail(`luminous/preparation: ${item.key} 核心工作区不可交互`)
      if (item.backgroundImage !== 'none') fail(`luminous/preparation: ${item.key} 仍使用装饰性渐变背景`)
    }
    if (railGeometry.rail.height > 44) fail(`luminous/preparation: 核心工作区 rail 高度异常（${railGeometry.rail.height}px）`)
    if (railGeometry.overflow > 2) fail(`luminous/preparation: 核心工作区 rail 横向溢出 ${railGeometry.overflow}px`)
  }

  const routes = ['paper', 'materials', 'match', 'check']
  for (const section of routes) {
    await preparationLuminous.locator(`.preparation-business-rail > button[data-section-key='${section}']`).click()
    await preparationLuminous.locator(`.preparation-workspace[data-section='${section}']`).waitFor({ state: 'visible', timeout: 10000 })
    if (section === 'match') await preparationLuminous.locator('.journal-match-layout').waitFor({ state: 'visible', timeout: 10000 })
    await preparationLuminous.locator(".preparation-business-rail > button[data-section-key='overview']").click()
  ...
  await preparationLuminous.locator('.journal-center-workspace').waitFor({ state: 'visible', timeout: 15000 })
  const journalActive = await activeStyle(preparationLuminous, 'journals')
  compareStyles('luminous active navigation', journalActive, preparationActive, ['color', 'backgroundColor', 'backgroundImage', 'borderColor', 'borderRadius', 'boxShadow', 'height'])
  details.push('luminous compact business rail, Journal Match workspace and Figure Composer entry remain clickable')
} catch (error) {
