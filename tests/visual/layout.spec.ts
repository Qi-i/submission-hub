...
test('dashboard layout geometry is consistent', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 })
  await page.goto(url)
  await page.locator("html[data-visual-ready='true'] .paper-card-v3").first().waitFor({ state: 'visible' })
  ...
  const result = await page.evaluate(() => {
    ...
    const status = card.querySelector<HTMLElement>('.paper-status-area > .badge')
    const pill = card.querySelector<HTMLElement>('.journal-pill')
    const icon = card.querySelector<HTMLElement>('.journal-pill-icon')
    const text = card.querySelector<HTMLElement>('.journal-pill-text')
