...
try {
  ...
  const widthInput = layout.getByLabel('单图宽度', { exact: true })
  const gapInput = layout.getByLabel('图间距', { exact: true })
  ...
  const widthA = Number(await inspector.getByRole('spinbutton', { name: 'W', exact: true }).inputValue())
  const xA = Number(await inspector.getByRole('spinbutton', { name: 'X', exact: true }).inputValue())
  await layers.nth(1).click()
  const widthB = Number(await inspector.getByRole('spinbutton', { name: 'W', exact: true }).inputValue())
  const xB = Number(await inspector.getByRole('spinbutton', { name: 'X', exact: true }).inputValue())
