  ...
  try {
    ...
    const result = await page.evaluate(referenceStyle => {
      ...
      cards.forEach((card, index) => {
        ...
        const accentHeight = parseFloat(accentStyle.height)
        if (!Number.isFinite(accentHeight) || accentHeight < 3.5 || accentHeight > 4.5) localFailures.push(`journal ${index + 1}: semantic accent line is outside strengthened Journal Center scale (${Number.isFinite(accentHeight) ? accentHeight.toFixed(1) : 'NaN'}px)`)
