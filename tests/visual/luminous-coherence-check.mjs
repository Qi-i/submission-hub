...
async function inspectPreparation(ui, theme) {
  ...
  try {
    ...
    if (beforeHover) await page.mouse.move(1432, Math.min(980, beforeHover.y + beforeHover.height / 2))
    ...
    const modalLayer = await page.evaluate(() => {
      ...
      return {
        ...
        switchOpacity: switcher ? Number.parseFloat(getComputedStyle(switcher).opacity) : 0,
      }
    })
...
try {
  ...
  writeFileSync('focused-review/luminous-coherence-report.json', JSON.stringify({ failures, details }, null, 2))
