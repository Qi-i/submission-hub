...
| ✨ 快速识别期刊 | 可通过 DOI、doi.org 链接或 ISSN 从 Crossref 识别期刊名称、出版社和 ISSN；在线版可通过服务端代理查询 EasyScholar 等级 |
| ⚖️ 期刊比较 | 最多并列比较 4 本期刊的质量、费用、速度、开放获取、接收率、收录和风险信息 |
| 🔄 流程衔接 | 选题可一键建立草稿，草稿可一键转入正式投稿管理并带出主投期刊和费用信息 |
...
1. 在 Supabase SQL Editor 中按编号依次执行 `supabase/001` 至 `supabase/010` 迁移文件。
...
4. 如需 EasyScholar 等级查询，在 Supabase Edge Function Secrets 中设置 `EASYSCHOLAR_SECRET_KEY`。该密钥只保存在服务端，不应写入前端代码或 GitHub 仓库。

`008_preparation_workspace.sql` 创建期刊库、研究选题和草稿准备三张表；`009_preparation_performance.sql` 补充草稿转投稿关联索引；`010_journal_rank_cache.sql` 增加期刊等级快照、缓存和每秒 2 次以内的服务端限流。
...
离线版不包含登录、Supabase 云同步、EasyScholar 密钥和 R2 上传。投稿记录、期刊库、选题、草稿和个人设置均保存在当前浏览器本地存储中。DOI / ISSN 基础识别属于可选联网工具，断网时不影响本地管理功能。
