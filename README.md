...
`008_preparation_workspace.sql` 创建期刊库、研究选题和草稿准备三张表；`009_preparation_performance.sql` 补充草稿转投稿关联索引；`010_journal_rank_cache.sql` 增加期刊等级快照、缓存和每秒 2 次以内的服务端限流；`011_external_api_hardening.sql` 为缓存与限流表增加显式拒绝策略并清理无用索引。
