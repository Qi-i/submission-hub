...
| 🔗 期刊数据互通 | 投稿记录按规范化期刊名自动关联期刊库；投稿卡片读取最新分区与官网，编辑投稿时可自动带出或手动同步期刊库信息 |
...
| 💱 APC 人民币参考价 | 保留原始金额和币种，外币 APC 在期刊总览、期刊库、编辑表单和比较页显示约合人民币、汇率日期与缓存状态；投稿管理卡片不外显费用 |
...
- 投稿管理卡片移除 APC 与费用备注；费用仅在期刊决策、期刊比较和编辑界面显示。
- 期刊库与投稿管理按规范化期刊名互通，投稿卡片实时读取最新期刊分区和官网，编辑投稿时可同步最新期刊档案。
- 外币 APC 使用公开日汇率显示人民币参考价，保留原始金额，优先使用 Frankfurter v2 接口并以 v1 接口回退，缓存 24 小时，断网时尝试使用最近缓存。
