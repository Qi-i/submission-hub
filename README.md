...
1. 在 Supabase SQL Editor 中按编号依次执行 `supabase/001` 至 `supabase/009` 迁移文件。
...
`008_preparation_workspace.sql` 创建期刊库、研究选题和草稿准备三张表；`009_preparation_performance.sql` 补充草稿转投稿关联索引。未执行 008 时，原有投稿管理仍可使用，但投稿准备页面会显示初始化提示。
...
离线版不包含登录、Supabase 云同步和 R2 上传。投稿记录、期刊库、选题、草稿和个人设置均保存在当前浏览器本地存储中，建议定期使用“备份”按钮导出完整 JSON 文件。
