...
const prepPath = 'src/components/PreparationWorkspace.tsx'
const onlinePath = 'src/components/OnlinePreparationWorkspace.tsx'
const stylesPath = 'src/app-styles.ts'
const checkPath = '.github/workflows/check.yml'
...
if (!prep.includes("const FigureComposer = lazy(() => import('./figure-composer/FigureComposer'))")) {
  prep = replaceRequired(prep, "import { useEffect, useMemo, useState } from 'react'", "import { lazy, Suspense, useEffect, useMemo, useState } from 'react'", 'Preparation React import')
  prep = replaceRequired(prep, "  FilePenLine, LayoutDashboard, Lightbulb, LoaderCircle, Plus, Scale, Search, Star, Target,", "  ClipboardCheck, FilePenLine, Images, LayoutDashboard, Lightbulb, LoaderCircle, PackageCheck, Plus, Scale, Search, Star, Target,", 'Preparation icons')
  prep = replaceRequired(prep, "import { useTheme } from '../lib/theme'\n\ntype SectionKey = 'overview' | 'topics' | 'drafts' | 'journals' | 'compare'", "import { useTheme } from '../lib/theme'\n\nconst FigureComposer = lazy(() => import('./figure-composer/FigureComposer'))\n\ntype SectionKey = 'overview' | 'paper' | 'figures' | 'materials' | 'match' | 'check' | 'topics' | 'drafts' | 'journals' | 'compare'", 'Preparation sections')
  prep = replaceRequired(prep, "  onPromoteDraft?: (draft: ManuscriptDraft) => Promise<void>\n  onLookupJournalRanks?: (publicationName: string) => Promise<JournalRankLookupResult>\n}", "  onPromoteDraft?: (draft: ManuscriptDraft) => Promise<void>\n  onLookupJournalRanks?: (publicationName: string) => Promise<JournalRankLookupResult>\n  onDraftFigureCountChange?: (draftId: string, count: number) => Promise<void> | void\n}", 'Figure count prop')
  prep = replaceRequired(prep, "  onSaveDraft, onDeleteDraft,\n  onPromoteDraft, onLookupJournalRanks,\n}: Props)", "  onSaveDraft, onDeleteDraft,\n  onPromoteDraft, onLookupJournalRanks, onDraftFigureCountChange,\n}: Props)", 'Figure count destructure')

  const oldNav = `    <div className="prep-nav">
  ...
  const newNav = `    <div className="prep-nav prep-nav-primary" aria-label="投稿准备核心工作区">
  ...
  prep = replaceRequired(prep, oldNav, newNav, 'Primary Preparation navigation')

  const anchor = "    {section === 'topics' && <div className=\"prep-card-grid\">"
  const inserted = `    {section === 'paper' && <div className="prep-primary-section-grid">
      <section className="prep-panel prep-panel-topic"><PanelHead title="选题与研究设计" subtitle="从研究问题推进到可执行论文草稿" onClick={() => setSection('topics')} /><div className="prep-topic-strip">{orderedTopics.slice(0, 5).map(topic => <TopicCard key={topic.id} topic={topic} onClick={() => setEditor({ type: 'topic', value: topic })} onCreateDraft={() => void createDraftFromTopic(topic)} creating={creatingTopicId === topic.id} compact />)}{!orderedTopics.length && <Empty text="尚无研究选题" action="新增选题" onClick={() => setEditor({ type: 'topic', value: 'new' })} />}</div></section>
    ...
    {section === 'match' && <><div className="prep-primary-section-head"><div><h2>期刊匹配</h2><p>把收藏期刊、关键分区、费用、审稿周期和横向比较放在同一工作区。</p></div><button className="btn btn-journal-primary btn-sm" onClick={() => setEditor({ type: 'journal', value: 'new' })}><Star size={13} /> 收藏期刊</button></div><div className="prep-card-grid journal-grid">{orderedJournals.map(journal => <JournalCard key={journal.id} journal={journal} onClick={() => setEditor({ type: 'journal', value: journal })} />)}{!orderedJournals.length && <Empty text={query ? '没有匹配的期刊' : '尚未收藏期刊'} action="收藏期刊" onClick={() => setEditor({ type: 'journal', value: 'new' })} />}</div>{!!orderedJournals.length && <JournalComparison journals={orderedJournals} onEdit={journal => setEditor({ type: 'journal', value: journal })} />}</>}
...
`
  prep = replaceRequired(prep, anchor, inserted + anchor, 'Primary Preparation content')
  fs.writeFileSync(prepPath, prep)
}

let online = fs.readFileSync(onlinePath, 'utf8')
if (online.includes("import FigureStudio from './FigureStudio'")) {
  online = replaceRequired(online, "import { useCallback, useEffect, useRef, useState } from 'react'", "import { lazy, Suspense, useCallback, useEffect, useRef, useState } from 'react'", 'Online React import')
  online = replaceRequired(online, "import FigureStudio from './FigureStudio'", "const FigureComposer = lazy(() => import('./figure-composer/FigureComposer'))", 'Online Figure Composer import')
  const promoteAnchor = "  const promoteDraft = async (draft: ManuscriptDraft) => {"
  const syncMethod = `  const syncDraftFigureCount = useCallback(async (draftId: string, count: number) => {
...
`
  online = replaceRequired(online, promoteAnchor, syncMethod + promoteAnchor, 'Figure count sync')
  online = replaceRequired(online, "? <FigureStudio onBack={() => setWorkspaceMode('preparation')} />", "? <Suspense fallback={<div className=\"prep-loading\"><div className=\"prep-loading-shell\"><div className=\"prep-loading-copy\"><strong>正在加载科研组图</strong><span>正在初始化本地图片工作区…</span></div></div></div>}><FigureComposer drafts={snapshot.drafts} onDraftFigureCountChange={syncDraftFigureCount} onBack={() => setWorkspaceMode('preparation')} /></Suspense>", 'Online Figure Composer render')
  online = replaceRequired(online, "onPromoteDraft={promoteDraft} onLookupJournalRanks={lookupJournalRanks} />}", "onPromoteDraft={promoteDraft} onLookupJournalRanks={lookupJournalRanks} onDraftFigureCountChange={syncDraftFigureCount} />}", 'Preparation callback')
  fs.writeFileSync(onlinePath, online)
}

let styles = fs.readFileSync(stylesPath, 'utf8')
if (styles.includes("import './figure-studio.css'")) {
  styles = styles.replace("import './figure-studio.css'", "import './figure-composer.css'")
  fs.writeFileSync(stylesPath, styles)
}

fs.rmSync('src/components/FigureStudio.tsx', { force: true })
fs.rmSync('src/figure-studio.css', { force: true })
fs.rmSync('.github/workflows/figure-composer-integration-patch.yml', { force: true })
fs.rmSync('scripts/apply-figure-composer-integration-patch.mjs', { force: true })

fs.writeFileSync(checkPath, `name: Check\n\non:\n  pull_request:\n    branches: [main]\n  push:\n    branches: [main, submission-workflow-enhancements, 'release/**']\n\npermissions:\n  contents: read\n\njobs:\n  build:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      - uses: actions/setup-node@v4\n        with:\n          node-version: 20\n          cache: npm\n      - name: Install locked dependencies\n        run: npm ci --no-audit --no-fund\n      - name: Run complete verification\n        run: npm run verify\n        env:\n          VITE_SUPABASE_URL: https://example.supabase.co\n          VITE_SUPABASE_ANON_KEY: build-verification-key\n          VITE_ADMIN_ID: build-verification-admin\n`)

console.log('Figure Composer integration patch applied and temporary scaffolding removed.')
